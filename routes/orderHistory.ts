...
const order = await ordersCollection.find({ email: updatedEmail }).toArray()
...
await ordersCollection.update({ _id: new ObjectId(req.params.id) }, { $set: { delivered: deliveryStatus, eta } })
